<?php
/**
* @SWG\Resource(
*  resourcePath="/help",
*  description="Help"
* )
*/
Route::group(['prefix' => 'help', 'middleware' => 'api.sign'], function () {

    /**
    * @SWG\Api(
    *     path="/api/help",
    *     @SWG\Operation(
    *      method="GET",
    *      nickname="help-list",
    *      summary="page list",
    *      notes="page list",
    *      type="array",
    *     items="$ref:Help",
    *      @SWG\Parameters(
    *          @SWG\Parameter(name="page", description="当前页", required=false, type="integer", paramType="query", defaultValue="1"),
    *          @SWG\Parameter(name="pageSize", description="页大小", required=false, type="integer", paramType="query", defaultValue="10"),
    *          @SWG\Parameter(name="sort", description="排序", required=false, type="string", paramType="query", defaultValue="id asc"),
    *          @SWG\Parameter(name="search", description="查询条件（数组的json格式, 键里面可带有比较符号，不带默认为: =）", required=false, type="string", paramType="query", defaultValue="{&quot;id >=&quot;:1}"),
    *          @SWG\Parameter(name="_sign", description="签名", required=true, type="string", paramType="query", defaultValue="****")
    *      )
    *    )
    * )
    */
    Route::get('/', ['as' => 'Help.index', 'uses' => 'HelpController@index']);

    /**
    * @SWG\Api(
    *     path="/api/help/{id}",
    *     @SWG\Operation(
    *      method="GET",
    *      nickname="help-show",
    *      summary="信息详情",
    *      notes="信息详情",
    *      type="Help",
    *      @SWG\Parameters(
    *          @SWG\Parameter(name="id", description="id", required=true, type="integer", paramType="path", defaultValue="1"),
    *          @SWG\Parameter(name="_sign", description="签名", required=true, type="string", paramType="query", defaultValue="****")
    *      )
    *  )
    * )
    */
    Route::get('/{id}', ['as' => 'Help.show', 'uses' => 'HelpController@show']);

    /**
    * @SWG\Api(
    *     path="/api/help",
    *     @SWG\Operation(
    *      method="POST",
    *      nickname="help-store",
    *      summary="新增",
    *      notes="新增",
    *      type="Help",
    *      @SWG\Parameters(
    *          @SWG\Parameter(name="content", description="", required=true,type="string", paramType="form", defaultValue="" ),
            *          @SWG\Parameter(name="created_at", description="", required=true,type="string", paramType="form", defaultValue="" ),
                    *          @SWG\Parameter(name="name", description="", required=false,type="string", paramType="form", defaultValue="" ),
            *          @SWG\Parameter(name="updated_at", description="", required=true,type="string", paramType="form", defaultValue="" ),
        *          @SWG\Parameter(name="_sign", description="签名", required=true, type="string", paramType="form", defaultValue="****")
    *      )
    *  )
    * )
    */
    Route::post('/', ['as' => 'Help.store', 'uses' => 'HelpController@store']);

    /**
    * @SWG\Api(
    *     path="/api/help/{id}",
    *     @SWG\Operation(
    *      method="POST",
    *      nickname="help-update",
    *      summary="更新",
    *      notes="更新",
    *      type="Help",
    *      @SWG\Parameters(
    *          @SWG\Parameter(name="content", description="", required=false,type="string", paramType="form", defaultValue="" ),
            *          @SWG\Parameter(name="created_at", description="", required=false,type="string", paramType="form", defaultValue="" ),
                    *          @SWG\Parameter(name="name", description="", required=false,type="string", paramType="form", defaultValue="" ),
            *          @SWG\Parameter(name="updated_at", description="", required=false,type="string", paramType="form", defaultValue="" ),
        *          @SWG\Parameter(name="id", description="id", required=true,type="integer", paramType="path", defaultValue="" ),
    *          @SWG\Parameter(name="_sign", description="签名", required=true, type="string", paramType="form", defaultValue="****")
    *      )
    *  )
    * )
    */
    Route::post('/{id}', ['as' => 'Help.update', 'uses' => 'HelpController@update']);

    /**
    * @SWG\Api(
    *     path="/api/help/{id}",
    *     @SWG\Operation(
    *      method="DELETE",
    *      nickname="help-delete",
    *      summary="删除",
    *      notes="删除",
    *      type="",
    *      @SWG\Parameters(
    *          @SWG\Parameter(name="id", description="id", required=true,type="integer", paramType="path", defaultValue="1" ),
    *          @SWG\Parameter(name="_sign", description="签名", required=true, type="string", paramType="query", defaultValue="****")
    *      )
    *  )
    * )
    */
    Route::delete('/{id}', ['as' => 'Help.delete', 'uses' => 'HelpController@destroy']);

});